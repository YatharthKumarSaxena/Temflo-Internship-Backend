...
const paginatedList = async (req, res) => {
  try {
    ...
    if (assigned.length > 0) {
      // Match tasks where assignedTo contains any of the given user IDs
    ...
    const resultsPromise = Task.find(baseQuery).skip(skip).limit(limit).sort(sortObject).exec();

    console.log('Executing query...');

    const countPromise = Task.countDocuments(baseQuery);
