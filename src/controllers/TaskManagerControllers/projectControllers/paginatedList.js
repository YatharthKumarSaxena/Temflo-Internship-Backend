...
const paginatedList = async (req, res) => {
  try {
    ...
    const allowedSortFields = ['createdAt', 'updatedAt', 'name', 'removed', '_id'];
    ...
    if (members.length > 0) {
      // Fetch projectIds where any of the members exist
      ...
      const projectIds = memberDocs.map((m) => m.projectId);

      // Only return projects that these members belong to
    ...
    const resultsPromise = Project.find(baseQuery).skip(skip).limit(limit).sort(sortObject).exec();

    console.log('Executing query...');

    const countPromise = Project.countDocuments(baseQuery);
